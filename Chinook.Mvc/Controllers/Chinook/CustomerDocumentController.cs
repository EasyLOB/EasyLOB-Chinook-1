...
namespace Chinook.Mvc
{
    public class CustomerDocumentController : BaseControllerSCRUDApplication<CustomerDocumentDTO, CustomerDocument>
    {
        ...
        public ActionResult Create(int? masterCustomerId = null)
        {
            ...
            // ???
            customerDocumentItemModel.CustomerDocument.FileAcronym = "?";
            ...
            try
            {
                if (ValidateModelState(Application.Repository))
                {
                    if (Request.Files.Count > 0)
                    {
                        ...
                        {
                            ZFileTypes fileType = LibraryHelper.GetFileType(Path.GetExtension(upload.FileName));
                            string acronym = LibraryHelper.GetAcronym(fileType);
                            customerDocumentItemModel.CustomerDocument.FileAcronym = acronym;
                            ...
                            if (Application.Create(customerDocumentItemModel.OperationResult, customerDocumentDTO))
                            {
                                byte[] uploadArray = new byte[upload.ContentLength];
                                upload.InputStream.Read(uploadArray, 0, uploadArray.Length);

                                IEdmEngine edmEngine = DependencyResolver.Current.GetService<IEdmEngine>();
                                edmEngine.WriteFile("CustomerDocument", customerDocumentDTO.CustomerDocumentId, fileType, uploadArray);
